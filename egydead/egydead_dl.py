...
class EgyDeadDL:
    ...
    def get_download_links(self, movie_url):
        ...
        # Pattern 2: .downloadv-item (Newer design)
        if not links:
            # Look for links inside .downloadv-item
            # <a href="..." class="downloadv-item"> ... <div class="name">Server Name</div> ... </a>
            items = re.findall(r'<a href="([^"]+)"[^>]*class="downloadv-item"[^>]*>(.*?)</a>', response.text, re.DOTALL)
            for href, content in items:
                name_match = re.search(r'<div class="name">(.*?)</div>', content)
                server_name = name_match.group(1).strip() if name_match else "Unknown Server"
                
                # Try to find quality in content or assume based on context
                quality = "Unknown"
        ...
        if not links:
             ...
             print("Debug: Saved page content to debug_movie_page.html")
