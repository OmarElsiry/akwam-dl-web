...
def test_series(series_name):
    ...
    print(f"  Found: {first_result.get(title_key, '').encode('utf-8', errors='replace')} ({first_result.get(link_key, '')})")
    ...
    print(f"  Testing episode: {ep_to_test.get(ep_title_key, '').encode('utf-8', errors='replace')} ({ep_to_test[ep_link_key]})")
    ...
    for q in qualities:
        q_link_key = 'url' if 'url' in q else 'link'
        print(f"    - {q['quality']}: {q[q_link_key]}")
        
    quality_to_test = qualities[0]
    q_link_key = 'url' if 'url' in quality_to_test else 'link'
    print(f"  Resolving quality: {quality_to_test['quality']} ({quality_to_test[q_link_key]})")
