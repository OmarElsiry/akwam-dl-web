...
async def get_final_mp4(url: str):
    async with async_playwright() as p:
        ...
        def handle_request(req):
            nonlocal mp4_url
            if ".mp4" in req.url or "downet.net" in req.url:
                mp4_url = req.url
        page.on("request", handle_request)

        # Hook to capture new pages (popups/redirects)
        async def handle_popup(popup):
            nonlocal mp4_url
            print("Popup opened:", popup.url)
            if ".mp4" in popup.url or "downet.net" in popup.url:
                mp4_url = popup.url
        ...
        try:
            ...
            if btn:
                ...
                # It might open a new tab, so we wait for the event
                await btn.click()
